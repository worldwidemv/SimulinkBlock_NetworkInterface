...
#include <signal.h>
#include "test_NetworkInterface_UDP_Output.h" // Model's header file
...
void abortHandler(int sig)
{
  ...
  rtmSetStopRequested(test_NetworkInterface_UDP_Output_M, 1);
}

//*
//  Thread handle of the base rate thread.
//  Fundamental sample time = 0.05s
...
#endif
{
  ...
  struct timespec period = { 0U, 50000000U };// 0.05 seconds
  ...
  // assign timing stats
  timing.fundamentelSampleTime = 0.05F;
  timing.meanSampleTime = 0.05F;

  // External mode
  rtSetTFinalForExtMode(&rtmGetTFinal(test_NetworkInterface_UDP_Ou_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(test_NetworkInterface_UDP_Ou_M->extModeInfo, 1,
      &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(test_NetworkInterface_UDP_Ou_M, true);
    }
  }
  ...
  while (rtmGetErrorStatus(test_NetworkInterface_UDP_Output_M) == NULL &&
         !rtmGetStopRequested(test_NetworkInterface_UDP_Output_M)) {
    // Check subrate overrun, set rates that need to run this time step

    // Trigger sub-rate threads

    // Execute base rate step
    test_NetworkInterface_UDP_Output_step();
    ...
    do {
      ...
      if (now.tv_sec > next.tv_sec ||
        ...
        uint32_T usec = (now.tv_sec - next.tv_sec) * 1000000 + (now.tv_nsec -
          next.tv_nsec)/1000;
        fprintf(stderr, "Base rate (0.05s) overrun by %d us\n", usec);
        timing.overruns++;
        next = now;
        continue;
      }
...
int_T main(int_T argc, const char_T *argv[])
{
  ...
  // Initialize model
  test_NetworkInterface_UDP_Output_initialize();
  ...
  // Terminate model
  test_NetworkInterface_UDP_Output_terminate();
  errStatus = rtmGetErrorStatus(test_NetworkInterface_UDP_Output_M);
...
// Local Variables:
// compile-command: "make -f test_NetworkInterface_UDP_Output.mk"
// End:
